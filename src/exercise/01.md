...
- [Passing Props](https://react.dev/learn/passing-props-to-a-component#step-2-read-props-inside-the-child-component) - use `{}` around the params in the function (component) declaration e.g. `function Avatar({ person, size }) {` not `function Avatar( person, size ) {`
